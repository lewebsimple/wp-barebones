...
# Rename project
sed -i -e "s/wp-barebones/${PROJECT}/g" .cpanel.yml .gitignore composer.json
...
if [ "${error_code-0}" -eq 0 ]; then
  ...
  rm -rf wp-content/dev-plugins/*
  rm -rf wp-content/prod-plugins/*
  ...
  git add -A
  git commit -am 'chore: initial wp-barebones project'
fi
